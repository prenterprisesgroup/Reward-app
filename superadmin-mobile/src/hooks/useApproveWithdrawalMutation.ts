      ...
      if (approvedItem) {
        queryClient.setQueriesData(
          ...
          (oldData: any) => {
            ...
            const newActivity = {
              id: `opt-${Date.now()}`,
              type: 'WITHDRAW_REQUEST', // or 'REWARD_DISTRIBUTED', backend might map it
              worker: approvedItem.worker.name,
              workerAvatar: approvedItem.worker.profilePhoto || null,
              amount: approvedItem.amount.toString(),
