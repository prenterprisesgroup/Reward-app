...
// Removed company admin hooks since superadmin will have different endpoints
...
export default function CompanyAdminDashboardScreen() {
  ...
  // Mock data for Superadmin until backend endpoints are ready
  const isStatsLoading = false;
  const isStatsError = false;
  const refetchStats = () => {};
  const statsData = {
    company: { name: 'Global Network', logo: '' },
    stats: { activeWorkers: 450, pendingWithdrawals: 12, qrBatchCount: 85, rewardsDistributed: 1250000 },
    trends: { workers: '+15%', withdrawals: '-2%', rewards: '+24%' }
  };

  const isActivityLoading = false;
  const activityData = { pages: [{ items: [] }] };

  const isPendingLoading = false;
  const pendingData = { pages: [{ withdrawals: [] }] };
    ...
    <Animated.View entering={FadeIn.delay(100)} style={[styles.headerContainer, { paddingTop: insets.top + 20 }]}>
      <View style={styles.headerLeft}>
        ...
        <View style={styles.badgeContainer}>
          <MaterialCommunityIcons name="shield-crown" size={16} color={theme.colors.success} />
          <Typography style={styles.badgeText} weight="medium">Super Admin</Typography>
        </View>
      </View>
